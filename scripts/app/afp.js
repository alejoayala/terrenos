angular.module('theme.afp', ['theme.core.services'])
  .controller('afpController', ['$scope', '$sce', '$modal', '$bootbox', '$window', '$http', '$theme', '$log', '$timeout', 'uiGridConstants', 'pinesNotifications', 'hotkeys', 
      'afpServices', 
    function($scope, $sce, $modal, $bootbox, $window, $http, $theme, $log, $timeout, uiGridConstants, pinesNotifications, hotkeys, 
      afpServices
      ){
    'use strict';
    shortcut.remove("F2"); $scope.modulo = 'afp';
    ...
    $scope.gridOptions = {
      ...
      multiSelect: true,
      columnDefs: [
        { field: 'id', name: 'idafp', displayName: 'ID', minWidth: 10,  sort: { direction: uiGridConstants.ASC} },
        { field: 'nombre_afp', name: 'nombre_afp', displayName: 'AFP', minWidth: 250 },
        { field: 'estado_afp', type: 'object', name: 'estado_afp', displayName: 'Estado', maxWidth: 250,
      ...
      onRegisterApi: function(gridApi) {
        ...
        $scope.gridApi.core.on.filterChanged( $scope, function(grid, searchColumns) {
          ...
          paginationOptions.searchColumn = {
            ...
            'nombre_afp' : grid.columns[2].filters[0].term
          }
          $scope.getPaginationServerSide();
        });
      }
    ...
    $scope.btnNuevo = function (size) {
      $modal.open({
        ...
        controller: function ($scope, $modalInstance, getPaginationServerSide) {
          ...
          $scope.fData = {};
          $scope.titleForm = 'Registro de afp';
         /* AUTOCOMPLETE CARGOS */ 
          $scope.getAfpAutocomplete = function(val) { 
            var params = {
              search: val,
              sensor: false
            }
            return afpServices.sListarAfpCbo(params).then(function(rpta) {
              var afp = rpta.datos.map(function(e) {
    ...
    $scope.btnHabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          afpServices.sHabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
    ...
    $scope.btnDeshabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          afpServices.sDeshabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
  ...
  .service("afpServices",function($http, $q) {
    ...
    function sHabilitar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"afp/habilitar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sDeshabilitar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"afp/deshabilitar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
