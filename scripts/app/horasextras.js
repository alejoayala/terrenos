angular.module('theme.horasextras', ['theme.core.services'])
  .controller('horasextrasController', ['$scope', '$filter' , '$sce', '$modal', '$bootbox', '$window', '$http', '$theme', '$log', '$timeout', 'uiGridConstants', 'pinesNotifications', 'hotkeys', 
      'horasextrasServices', 'controlempleadoServices' , 'empleadoServices' ,
    ...
    $scope.gridOptions = {
      ...
      multiSelect: true,
      columnDefs: [
        { field: 'id', name: 'idcontrolhoras', displayName: 'ID', minWidth: 10,  sort: { direction: uiGridConstants.ASC} },
      ...
      onRegisterApi: function(gridApi) {
        ...
        $scope.gridApi.core.on.filterChanged( $scope, function(grid, searchColumns) {
          ...
          paginationOptions.searchColumn = {
            'idcontrolhoras' : grid.columns[1].filters[0].term,
    ...
    $scope.getPaginationServerSide = function() {
      ...
      horasextrasServices.sListarAsistencias($scope.datosGrid).then(function (rpta) {
        console.log(rpta);
    ...
    $scope.btnIngreso = function (size) {
      $modal.open({
        templateUrl: angular.patchURLCI+'horasextras/ver_popup_formulario',
        size: size || '',
        backdrop: 'static',
        keyboard:false,
        controller: function ($scope, $modalInstance, getPaginationServerSide) {
         ...
         /* AUTOCOMPLETE CARGOS */ 
          ...
          $scope.aceptar = function () { 
            horasextrasServices.sRegistrar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $modalInstance.dismiss('cancel');
  ...
  .service("horasextrasServices",function($http, $q) {
    return({
    ...
    function sListarAsistencias(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"horasextras/lista_asistencia", 
            data : datos
      });
    ...
    function sListarAsistenciaPorEmpleado(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"horasextras/lista_asistencia_por_empleado", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sRegistrar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"horasextras/registrar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sEditar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"horasextras/editar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
