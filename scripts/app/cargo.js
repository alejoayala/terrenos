angular.module('theme.cargo', ['theme.core.services'])
  .controller('cargoController', ['$scope', '$sce', '$modal', '$bootbox', '$window', '$http', '$theme', '$log', '$timeout', 'uiGridConstants', 'pinesNotifications', 'hotkeys', 
      'cargoServices', 
    function($scope, $sce, $modal, $bootbox, $window, $http, $theme, $log, $timeout, uiGridConstants, pinesNotifications, hotkeys, 
      cargoServices
      ){
    'use strict';
    shortcut.remove("F2"); $scope.modulo = 'cargo';
    ...
    $scope.gridOptions = {
      ...
      multiSelect: true,
      columnDefs: [
        { field: 'id', name: 'idcargo', displayName: 'ID', minWidth: 10,  sort: { direction: uiGridConstants.ASC} },
        { field: 'nombre_cargo', name: 'nombre_cargo', displayName: 'Cargo', minWidth: 250 },
        { field: 'estado_ca', type: 'object', name: 'estado_ca', displayName: 'Estado', maxWidth: 250,
      ...
      onRegisterApi: function(gridApi) {
        ...
        $scope.gridApi.core.on.filterChanged( $scope, function(grid, searchColumns) {
          ...
          paginationOptions.searchColumn = {
            ...
            'nombre_cargo' : grid.columns[2].filters[0].term
          }
          $scope.getPaginationServerSide();
        });
      }
    ...
    $scope.getPaginationServerSide = function() {
      ...
      cargoServices.sListarCargos($scope.datosGrid).then(function (rpta) {
    ...
    $scope.btnNuevo = function (size) {
      $modal.open({
        ...
        controller: function ($scope, $modalInstance, getPaginationServerSide) {
         ...
         /* AUTOCOMPLETE CARGOS */ 
          $scope.getCargosAutocomplete = function(val) { 
            var params = {
              search: val,
              sensor: false
            }
            return cargoServices.sListarCargosCbo(params).then(function(rpta) {
              var cargos = rpta.datos.map(function(e) {
    ...
    $scope.btnHabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          cargoServices.sHabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
    ...
    $scope.btnDeshabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          cargoServices.sDeshabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
  ...
  .service("cargoServices",function($http, $q) {
    ...
    function sListarCargos(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"cargo/lista_cargos", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sListarCargosCbo(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"cargo/lista_cargos_cbo", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    ...
    function sHabilitar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"cargo/habilitar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sDeshabilitar (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"cargo/deshabilitar", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
