...
class Banco extends CI_Controller {
	...
	public function lista_bancos_cbo()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		if( isset($allInputs['search']) ){
			$lista = $this->model_banco->m_cargar_banco_cbo($allInputs);
		}else{
	...
	public function lista_bancos()
	{
		...
		$totalRows = $this->model_banco->m_count_banco();
		$arrListado = array();
		foreach ($lista as $row) {
			if( $row['estado_ba'] == 1 ){
				$estado = 'HABILITADO';
	...
	public function habilitar()
	{
    	...
    	$arrData['flag'] = 0;
    	foreach ($allInputs as $row) {
			if( $this->model_banco->m_habilitar($row['id']) ){
	...
	public function deshabilitar()
	{
    	...
    	$arrData['flag'] = 0;
    	foreach ($allInputs as $row) {
			if( $this->model_banco->m_deshabilitar($row['id']) ){
