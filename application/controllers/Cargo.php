...
class Cargo extends CI_Controller {
	...
	public function lista_cargos_cbo()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		if( isset($allInputs['search']) ){
			$lista = $this->model_cargo->m_cargar_cargo_cbo($allInputs);
		}else{
			$lista = $this->model_cargo->m_cargar_cargo_cbo();
		}
		$arrListado = array();
	...
	public function lista_cargos()
	{
		...
		$totalRows = $this->model_cargo->m_count_cargo();
		$arrListado = array();
		foreach ($lista as $row) {
			if( $row['estado_ca'] == 1 ){
				$estado = 'HABILITADO';
	...
	public function habilitar()
	{
    	...
    	$arrData['flag'] = 0;
    	foreach ($allInputs as $row) {
			if( $this->model_cargo->m_habilitar($row['id']) ){
	...
	public function deshabilitar()
	{
    	...
    	$arrData['flag'] = 0;
    	foreach ($allInputs as $row) {
			if( $this->model_cargo->m_deshabilitar($row['id']) ){
